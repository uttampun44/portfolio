                ...
                SidebarData.map((item, index: number) => {
                  return (
                    <li key={index} className="menuItem  gap-4 my-2">
                      <Link href={item.href} className="text-base font-medium text-backend-primary-text-color" onClick={() => handleToggle(item.id)}>{item.name}</Link>
                      <ul className={toggle ? "menuList" : "hidden"}>
                        {
                          item.subNav?.map((subItem, index: number) => {
                            return (
                              <li key={index}>
                                <Link href={subItem.href} key={index} className="text-base font-normal text-backend-primary-text-color">{subItem.name}</Link>
                              </li>
                            )
                          }
                          )}
